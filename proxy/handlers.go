...
func (s *ProxyServer) handleLoginRPC(cs *Session, params []string, id string) (bool, *ErrorReply) {
	...
	UserPass:=""
	if len(params) == 1{
		log.Println(" User with no password set: assumed to be blank")
	}
	if len(params) >= 2{ // Assuming that the password was the second parameter in the string passed by the client
		UserPass=params[1]
		log.Println(" Provided Password from the User:", UserPass)
	}
	if UserPass != s.pass || s.user != params[0]{ // password and login must match
	...
	if !util.IsValidHexAddress(login) {
		return false, &ErrorReply{Code: -1, Message: "Invalid login: The login must be a valid address"}
	}
