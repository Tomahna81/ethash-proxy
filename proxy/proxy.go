...
type ProxyServer struct {
	...
	upstream           int32
	upstreams          []*rpc.RPCStratumClient
...
func NewProxy(cfg *Config, backend *storage.RedisClient) *ProxyServer {
	...
	proxy.pass = cfg.Proxy.Password
	proxy.user = cfg.Proxy.Username
	proxy.upstreams = make([]*rpc.RPCStratumClient, len(cfg.Upstream)) // Mod Tomahna
	for i, v := range cfg.Upstream {
		//proxy.upstreams[i], err_login := rpc_stratum.NewRPCStratumClient(v.Name, v.Url, v.Timeout)  // Mod Tomahna
		proxy.upstreams[i] = rpc.NewRPCStratumClient(v.Name, v.Url, v.Port, v.Username, v.Timeout)  // Mod Tomahna
		log.Printf("Upstream Pool: %s => %s with username: %s", v.Name, v.Url, v.Username)			
		//if err_login !=nil{
		//	proxy.upstreams[i].markSick()
		//	log.Printf(" Loging to the pool failed. The pool is marked as sick... Vigilance from the operator required")		
		//}
	}
	log.Printf("Default upstream Pool: %s => %s", proxy.rpc_stratum().Name, proxy.rpc_stratum().Url) // Mod Tomahna
	...
	proxy.fetchBlockTemplate() // The first fetch has no share submited and must retrieve a new job
	...
	go func() {
		for {
			select {
			case <-refreshTimer.C:
				proxy.fetchBlockTemplate() // The default block update is use getwork
...
func (s *ProxyServer) rpc_stratum() *rpc.RPCStratumClient {
